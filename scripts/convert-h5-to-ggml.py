...
json.load((dir_model / "added_tokens.json").open("r", encoding="utf8"))
...
mel_path = dir_whisper / "whisper" / "assets" / "mel_filters.npz"
...
for i in range(filters.shape[0]):
    for j in range(filters.shape[1]):
        fout.write(struct.pack("f", filters[i][j].item()))
